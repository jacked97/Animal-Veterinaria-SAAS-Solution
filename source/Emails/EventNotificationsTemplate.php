...
class EventNotificationsTemplate
{

    private $templatePath = '../Emails/Templates/event-notification.html';
    private $subject = 'la notifica evento imminente';
    private $template = '';

    private $ATTR_NOTIFICATION_TIME = "{{NOTIFICATION-TIME}}";
    ...
    public function sendEmail($calendarObject, $to)
    {
        ...
        $this->template = str_replace($this->ATTR_NOTIFICATION_TIME, $calendarObject->company->alert, $this->template);
    ...
    public function updateDatabase($calendarObj)
    {
        $calendarObj->notified = 1;
        $calendarObj->save();
    }
}